...
- M6: Operational Concerns
- M7: Reorganization / Editorial Consolidation

M7 goal: reorganize and consolidate existing material only. No new technical themes are introduced here.
M8 will finalize diagrams, figure numbering, and the comparison table.
-->
...
> **Figure 1 — Before and after: introducing the probabilistic layer**
...
> **Figure 5 — Technical, Agent, and Business observability**
...
- **Agent:** prompt or instruction context, selected tool, inferred parameters, workflow decisions
...
> *This table will be finalized in Milestone 8.*

| Aspect | Traditional Integration | Agent-Ready Integration |
|---|---|---|
| Consumer | Frontend / Microservice | Agent Runtime |
| Interaction | API contract | Tool / MCP contract over enterprise APIs |
| Interpretation | Deterministic | Probabilistic decision layer |
| Business authority | Backend | Backend |
| Validation | Important | Critical and server-side |
| Authorization | User/service identity | Delegated/service identity + tool + domain authorization |
| Interface design | Domain-oriented | Semantic agent-facing tools |
| Idempotency | Recommended | Critical for side effects and retries |
| Ambiguity | Client/application handled | Explicit semantic states |
| Context trust | Application-specific | Retrieved/tool context treated as untrusted |
| Rate limiting | Per user/service | User + agent/session + tool |
| Cost control | Infrastructure-level | Execution budgets + per-tool governance |
| Observability | Technical | Technical + Agent + Business |
| Testing | Functional | Functional + Contract + Agent Evals |
| Versioning | API/schema | Schema + description + behavioral contract |
| Long operations | Sync or application-specific async | Explicit job/status workflow |
